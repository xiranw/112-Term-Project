# starting, editing, classic playing, pvp playing, and ending screen logic
...
def buttonClicked(self, button):
    if button == "classicButton":
        x1, y1 = self.screenWidth/6, 400
        deltaX, deltaY = self.classicButtonSize
    elif button == "pvpButton":
        x1, y1 = self.screenWidth/6, 700
        deltaX, deltaY = self.pvpButtonSize
    elif button == "editorButton":
        x1, y1 = self.screenWidth*1.4, 400
...
def runStartScreen(self):
    ...
    while onStartScreen:
        ...
        if not drawDepthWarning(self) and classicClicked(self):
            onStartScreen = False
            self.classic = True
        elif not drawDepthWarning(self) and pvpClicked(self):
            onStartScreen = False
            self.pvp = True
...
def drawStartText(self):
    ...
    self.classicButtonSize = drawButton(self, "CLASSIC", titleFont, "topleft",
                                     white, black, (self.screenWidth/6, 400))
    
    self.pvpButtonSize = drawButton(self, "P. V. P.", titleFont, "topleft",
                                    white, black, (self.screenWidth/6, 700))
    
    self.editorButtonSize = drawButton(self, "EDITOR", titleFont, "topleft",
                                     white, black, (self.screenWidth*1.4, 400))
                                     

### Navigation to classic, pvp, or Level Editor

def classicClicked(self):
    return buttonClicked(self, "classicButton")

def pvpClicked(self):
    return buttonClicked(self, "pvpButton")
...
def pvpPlayScreen(self):
    ...
    self.gameOver = False
    self.timeLeft = 40
    while not self.pvpGameOver:
        self.timeLeft -= self.clock.tick()/1000
        if self.timeLeft <= 0:
            self.pvpGameOver = True
        ...
        if self.targetCircles == []: #game begins
            Shapes.pvpGenerateTargets(self)
        elif Shapes.pvpShapeComplete(self) == (1,0):
            self.pvpScores[0] += 10 * len(self.targetCircles[0])
            Shapes.pvpGenerateTargets(self)
        elif Shapes.pvpShapeComplete(self) == (0,1):
            self.pvpScores[1] += 10 * len(self.targetCircles[0])
            Shapes.pvpGenerateTargets(self)
        
        try:
            if drawDepthWarning(self)[0] == True and self.pvpScores[0] > 0.2:
                self.pvpScores[0] -= 0.2
            elif drawDepthWarning[1] == False and self.pvpScores[1] > 0.2:
                self.pvpScores[1] -= 0.2
        except:
            pass
        
        Shapes.pvpUpdateTargets(self)
        Shapes.generateBodyCircles(self)
        Shapes.pvpCheckCollisions(self)
        Shapes.pvpDrawShapes(self)
        
        pvpDrawText(self)
        self.adjustKinectFrame()
        pygame.display.update()
        
def pvpDrawText(self):
    pygame.draw.line(self.frameSurface, black, (960, 70), (960, 1080), 5)
    #draw score
    score1 = smallFont.render("SCORE: " + str(int(self.pvpScores[0])), True, white, black)
    score1Rect = score1.get_rect(center=(self.screenWidth/2, 150))
    self.frameSurface.blit(score1, score1Rect)
    
    score2 = smallFont.render("SCORE: " + str(int(self.pvpScores[1])), True, white, black)
    score2Rect = score1.get_rect(center=(self.screenWidth*3/2, 150))
    self.frameSurface.blit(score2, score2Rect)
    
    #draw timeLeft
    timeLeft = smallFont.render("Time: %0.2f" %self.timeLeft, True, white, black)
    timeRect = timeLeft.get_rect(center=(self.screenWidth, 50))
    self.frameSurface.blit(timeLeft, timeRect)
...
def pvpEndText(self):
    winner = checkWinner(self)
    
    if winner != "tie":
        over = titleFont.render("Game Over! " + winner + " wins!", True, black)
        overRect = over.get_rect(center=(self.screenWidth, 100))
        self.frameSurface.blit(over, overRect)
    else:
        over = titleFont.render("Game Over! You two tied!", True, black)
        overRect = over.get_rect(center=(self.screenWidth, 100))
        self.frameSurface.blit(over, overRect)
...
def checkWinner(self):
    if int(self.pvpScores[0]) > int(self.pvpScores[1]):
        return "Player 1"
    elif int(self.pvpScores[0]) < int(self.pvpScores[1]):
        return "Player 2"
    return "tie"
