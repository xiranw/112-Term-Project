...
def pvpGenerateTargets(self, numOfTargets=None):
    if numOfTargets == None:
        numOfTargets = random.choice((2,3,4))
    newTargets1 = []
    newTargets2 = []
    for i in range(numOfTargets):
        targetX = random.randint(250, 750)
        targetY = random.randint(250, 900)
        newTargets1.append(targetCircle((targetX, targetY)))
    if redoTargets(newTargets1):
        newTargets1 = pvpGenerateTargets(self, numOfTargets)
    for target1 in newTargets1:
        target2X = target1.pos[0] + 960
        target2Y = target1.pos[1]
        newTargets2.append(targetCircle((target2X, target2Y)))
    self.targetCircles = [newTargets1, newTargets2]
    return newTargets1

def pvpUpdateTargets(self):
    for shape in self.targetCircles:
        for targetCircle in shape:
            targetCircle.hit = False
    
def pvpCheckCollisions(self):
    for shape in self.targetCircles:
        for targetCircle in shape:
            for bodyCircle in self.bodyCircles:
                if targetCircle.isHit(bodyCircle):
...
def pvpShapeComplete(self):
    counter1, counter2 = 0, 0
    for targetCircle in self.targetCircles[0]:
        if targetCircle.hit == True:
            counter1 += 1
    if counter1 == len(self.targetCircles[0]):
        return (1,0)
    
    for targetCircle in self.targetCircles[1]:
        if targetCircle.hit == True:
            counter2 += 1
    if counter2 == len(self.targetCircles[0]):
        return (0,1)
    
def pvpDrawShapes(self):
    for shape in self.targetCircles:
        polygon = Polygon(shape)
        polygon.draw(self.frameSurface)
        for circle in shape:
            circle.draw(self.frameSurface)
    for circle in self.bodyCircles:
        circle.draw(self.frameSurface)
